...
class ConvLayer(object):
    ...
    def bp_sensitivity_map(self , sensitivity_array , activator):
        ...
        #将计算结果与激活函数的偏导数做element-wise惩罚操作
    ...
    #对步长不为1的sensitivity map进行扩展，使之还原成stride=1时的情况
    def expand_sensitivity_map(self , sensitivity_array):
        depth = sensitivity_array.shape[0]
        #确定扩展后sensitivity map的大小
        #计算stride为1时的sensitivity map的大小，之所以这么做是因为后面对于stride不等于1的情况时，进行反向传播时，都是先还原成stride=1时的情况再做处理
        ...
        for i in range(self.output_height):
            for j in range(self.output_width):
                i_pos = i * self.stride
                j_pos = j * self.stride
                expand_array[: , i_pos , j_pos] = sensitivity_array[: , i , j]  #stride = s还原到stride=1时的情况，通过对应位置0进行扩展
        return expand_array
